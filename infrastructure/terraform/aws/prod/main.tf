...
locals {
  ...
  non_gov_network_cird_blocks   = {
    ...
    deekshana_dev = {
      ...
      cidr_blocks  = ["80.233.39.211/32"]
    },
    sina_dev = {
      description = "Sina - Sidero"
      cidr_blocks  = ["148.251.150.109/32"]
    },
...
resource "aws_acm_certificate" "cib_content" {
  domain_name                 = "prodcib.testing.gov.ie"
  subject_alternative_names = [
     "www.prodcib.testing.gov.ie"
  ]
  validation_method           = "DNS"
...
resource "aws_acm_certificate" "cib_content_admin" {
  domain_name                 = "prodcontent.cib.testing.gov.ie"
  validation_method           = "DNS"
